...
export function BlockedSlotsForm({
  ...
  return (
    <div className="space-y-5">
      <div className="rounded-[20px] bg-[var(--color-panel)] p-4">
        <p className="font-medium text-[var(--color-ink)]">Что это такое</p>
      ...
      <form action={action} className="space-y-4 rounded-[20px] border border-[var(--color-line)] p-4">
        ...
        {blockedSlots.length > 0 ? (
          blockedSlots.map((slot) => {
            ...
            return (
              <div
                key={slot.id}
                className="rounded-[20px] border border-[var(--color-line)] p-4"
              >
                <div className="flex flex-wrap items-start justify-between gap-3">
                  <div className="space-y-1">
                    <p className="font-medium text-[var(--color-ink)]">{slot.reason}</p>
                  ...
                  <form action={deleteBlockedSlotAction}>
                    ...
                    <button
                      type="submit"
                      className="inline-flex min-h-10 items-center gap-2 rounded-full border border-[var(--color-line)] px-4 py-2 text-sm font-medium text-[var(--color-ink)] transition hover:bg-[var(--color-panel)]"
                    >
                      <Trash2 size={14} />
                      Удалить
                    </button>
          ...
          <div className="rounded-[20px] border border-dashed border-[var(--color-line)] p-4 text-sm text-[var(--color-muted)]">
